...
(function () {
  ...
  function isJkItem(item) {
    ...
    var f = String(item.fuente || item.source || "").toLowerCase();
    if (f.indexOf("jkanime") !== -1 || f === "jk") return true;
    var blob = String(item.link || item.url || item.url_extract || "").toLowerCase();
  ...
  function hideKoiServerUi() {
    try {
      ...
      if (view) view.classList.add("mz-jk-direct");
      var boxN = $("mz-kp-servers");
      var boxD = $("mz-kp-servers-direct");
      var labD = $("mz-kp-direct-label");
      var wraps = document.querySelectorAll("#mz-koi-ep-view .mz-kp-servers");
      ...
      wraps.forEach(function (wrap) {
        if (wrap.classList.contains("mz-kp-downloads-wrap")) return;
  ...
  function showKoiServerUi() {
    try {
      var view = $("mz-koi-ep-view");
      if (view) view.classList.remove("mz-jk-direct");
      document.querySelectorAll("#mz-koi-ep-view .mz-kp-servers[data-jk-hidden='1']").forEach(function (wrap) {
...
`;
  ...
  async function openEpisode(item, episodio, seasonNum, epNum) {
    ...
    try {
      ...
      destroyHls();
      showPoster(item, isJkItem(item) ? "Cargando reproductor…" : "Elige un reproductor para comenzar");
      if (isJkItem(item)) hideKoiServerUi();
      else showKoiServerUi();
      renderServers([]);
      renderDownloads([]);
